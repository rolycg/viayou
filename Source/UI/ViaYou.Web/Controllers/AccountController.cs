...
namespace ViaYou.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginViewModel model, string returnUrl)
        {
            ...
            var result = SignInManager.PasswordSignIn(user.UserName, model.LoginPassword, model.RememberMe, shouldLockout: false);
            switch (result)
            {
        ...
        public ActionResult Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var userName = model.RegisterEmail.Split('@')[0];
                var user = new ApplicationUser { UserName = userName, Email = model.RegisterEmail };
                var result = UserManager.Create(user, model.RegisterPassword);
                if (result.Succeeded)
                {
